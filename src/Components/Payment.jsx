...
const Payment = () => {
  ...
  useEffect(() => {
    ...
    // Fetch customer details from local storage
    const loggedInUser = JSON.parse(localStorage.getItem("loggedInUser"));
    if (loggedInUser) {
      const randomPhone = `9${Math.floor(Math.random() * 1000000000)}`; // Random 10-digit Indian phone number
      const randomAddress = `${
        ["123 Main St", "456 Elm St", "789 Oak St"][Math.floor(Math.random() * 3)]
      }, ${
        ["Mumbai", "Delhi", "Bangalore"][Math.floor(Math.random() * 3)]
      }, India - ${Math.floor(Math.random() * 900000) + 100000}`; // Random address with pin code
  ...
  const generateInvoice = () => {
    ...
    doc.text(`Email: ${customerDetails.email}`, 120, 25);
    doc.text(`Phone: ${customerDetails.phone}`, 120, 35);
    doc.text(`Address: ${customerDetails.address}`, 120, 45);
    doc.line(10, 55, 200, 55);

   
    doc.setFontSize(18);
    doc.text("Invoice", 90, 65);
