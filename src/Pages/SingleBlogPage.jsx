...
const SingleBlogPage = () => {
  ...
  const filteredBlogs = blogs.filter(
    (blog) => blog.title === decodeURIComponent(title)
  );
  ...
  return (
    <>
      <Header />
      <Flex direction={{ base: "column", md: "row" }} gap={6} py={10} px={{base: 4, md :8 ,lg :24}}>
        <BlogDetail filteredBlogs={filteredBlogs} flex="2" />
        <ProductCards flex="1" />
      </Flex>
      <Footer />
    </>
